...
class Products extends Admin_Controller {	
	...
	function product_image_upload()
	{
		...
		if ( $this->upload->do_upload())
		{
			...
			if($upload_data['image_width']=='900' && $upload_data['image_height']=='900'){
				/*
				...
				*/			
				...
				$data['error'] = 'Image Size does not match. (900 x 900)';
			}
		}
		
		$this->upload->set_upload_path('uploads/product/thumb');
		if ( $this->upload->do_upload('userfile_thumb',$upload_data['file_name']))
		{
			$upload_data	= $this->upload->data();
			
			$this->load->library('image_lib');
			
			if($upload_data['image_width']=='190' && $upload_data['image_height']=='240'){
				$data['file_name']	= $upload_data['file_name'];
			}else{
				$data['error'] = 'Image Size does not match. (190px x 240px)';
			}
		}
