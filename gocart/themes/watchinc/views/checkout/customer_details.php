...
?>
	...
	<div id="customer_error_box" class="error" style="display:none"></div>
		<h3 style="margin-top: 0; margin-bottom: 20px;"><?php echo lang('customer_information');?></h3>
		<form class="form-horizontal"	 id="customer_info_form">
			<div class="thing">
				...
				<div class="form-group">
					<div class="col-sm-12">
						<button type="button" class="btn btn-block btn-black" onclick="save_customer();" style="border-radius: 0; margin-top: 20px;">UPDATE ADDRESS</button>
					</div>
				</div>
			</div>
		</form>
				...
				foreach($customer_addresses as $a):?>
					<div class="my_account_address" id="address_<?php echo $a['id'];?>">
						<div style="position: relative;">
							<div style="padding: 10px; margin-top: 15px; background-color: #ddd;">
								<?php
								$b	= $a['field_data'];
							...
							<div class="address_toolbar">
								<button type="button" class="btn btn-black btn-sm choose_address" onclick="populate_address(<?php echo $a['id'];?>); $.colorbox.close()" style="border-radius: 0; position: absolute; right: 0; top: 0;"><?php echo lang('form_choose');?></button>
							</div>
						</div>
					</div>
				<?php endforeach;?>
